...
def main() -> None:
    p=argparse.ArgumentParser(); p.add_argument("--suite",choices=("full","ablation"),required=True); p.add_argument("--manifest",type=Path,default=Path("outputs/manifests/jobs.json")); p.add_argument("--execute-template"); p.add_argument("--execute",action="store_true"); a=p.parse_args()
